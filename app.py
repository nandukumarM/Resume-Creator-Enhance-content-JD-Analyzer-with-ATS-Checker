...
if IS_VERCEL:
    ...
    app.config['ANALYSIS_FOLDER'] = '/tmp/analysis_data'
    PHOTO_DIR = '/tmp/static/uploads/photos'
else:
...
if IS_VERCEL:
    ...
    for filename in ['users.json', 'admins.json', 'history.json', 'database.db']:
        ...
        if os.path.exists(src) and not os.path.exists(dst):
            try:
                shutil.copy(src, dst)
                print(f"Copied bundled {filename} to {dst}")
            except Exception as e:
...
def admin_register():
    if request.method == 'POST':
        ...
        if secret_key != ADMIN_SECRET_KEY:
            error_msg = "Oops! 🕵️‍♂️ Looks like you forgot the secret handshake! Enter the correct Secret Key to enter the batcave!"
            return render_template('admin_register.html', error=error_msg)
        ...
        if hasattr(ws, 'generate_password_hash'):
            admins[username]['password'] = ws.generate_password_hash(password)
        else:
            admins[username]['password'] = password

        save_admins(admins)
        session['admin'] = admins[username]
...
def admin_login():
    if request.method == 'POST':
        ...
        if hasattr(ws, 'check_password_hash'):
            try:
                is_valid = ws.check_password_hash(admin['password'], password)
            except:
